...
async def demo_mcp_server():
    ...
    try:
        ...
        for concept, action in concepts:
            DemoOutput.tool_call(
                "cognitive_learn", {"action": action, "concept": concept}
            )
            result = await server.execute_tool(
                "cognitive_learn",
                {"action": action, "concept": concept, "strength": 0.95},
            )
        ...
        # Query a concept
        DemoOutput.tool_call(
            "cognitive_query", {"query_type": "concept", "concept": "Python"}
        )
        result = await server.execute_tool(
            "cognitive_query", {"query_type": "concept", "concept": "Python"}
        )
        ...
        # Query relationships
        DemoOutput.tool_call(
            "cognitive_query", {"query_type": "relationships", "concept": "TensorFlow"}
        )
        result = await server.execute_tool(
            "cognitive_query", {"query_type": "relationships", "concept": "TensorFlow"}
        )
        ...
        DemoOutput.tool_call(
            "cognitive_reason", {"mode": "deduction", "max_inferences": 10}
        )
        result = await server.execute_tool(
            "cognitive_reason", {"mode": "deduction", "max_inferences": 10}
        )
        ...
        # Focus on a concept
        DemoOutput.tool_call(
            "cognitive_focus", {"action": "focus", "concept": "Machine Learning"}
        )
        result = await server.execute_tool(
            "cognitive_focus",
            {"action": "focus", "concept": "Machine Learning", "amount": 0.8},
        )
        DemoOutput.success(result.content[0]["text"])

        # Spread attention
        DemoOutput.tool_call("cognitive_focus", {"action": "spread"})
        result = await server.execute_tool("cognitive_focus", {"action": "spread"})
        DemoOutput.success(result.content[0]["text"])

        # Get attention status
        DemoOutput.tool_call("cognitive_focus", {"action": "status"})
        result = await server.execute_tool("cognitive_focus", {"action": "status"})
        DemoOutput.result(result.content[0]["text"])
        ...
        result = await server.execute_tool(
            "cognitive_reflect",
            {"include_optimizations": True, "apply_optimizations": False},
        )
        DemoOutput.result(result.content[0]["text"])
        ...
        result = await server.execute_tool("cognitive_status", {"verbose": True})
        DemoOutput.result(result.content[0]["text"])
