...
class AtomSpaceBenchmarks:
    ...
    def bench_link_creation(self) -> BenchmarkResult:
        ...
        def create_link():
            ...
            atomspace.add_link(
                AtomType.INHERITANCE_LINK, [nodes[i], nodes[j]], tv=TruthValue(0.9, 0.9)
            )
...
class PLNBenchmarks:
    ...
    def bench_deduction(self) -> BenchmarkResult:
        ...
        atomspace.add_link(AtomType.INHERITANCE_LINK, [a, b], tv=TruthValue(0.9, 0.9))
    ...
    def bench_stimulation(self) -> BenchmarkResult:
        ...
        for i in range(100):
            h = atomspace.add_node(
                AtomType.CONCEPT_NODE, f"Stim_{i}", av=AttentionValue(sti=0.3, lti=0.5)
            )
            atoms.append(h)
        ...
        def worker(worker_id):
            try:
                for i in range(100):
                    ...
                    atomspace.get_atoms_by_type(AtomType.CONCEPT_NODE)
                    # Write
                    atomspace.add_node(AtomType.CONCEPT_NODE, f"Worker_{worker_id}_{i}")
                    with lock:
                        operations[0] += 2
