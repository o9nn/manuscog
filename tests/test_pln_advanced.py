...
from kernel.reasoning.pln import PLNConfig, PLNEngine, PLNFormulas
...
class TestPLNDeduction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        config = PLNConfig(max_depth=10, confidence_threshold=0.1)
    ...
    def test_simple_deduction(self):
        ...
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [a, b], tv=TruthValue(0.9, 0.95)
        )
        # B -> C with high confidence
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [b, c], tv=TruthValue(0.85, 0.9)
        )
    ...
    def test_chained_deduction(self):
        ...
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [human, mammal], tv=TruthValue(1.0, 0.99)
        )
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [mammal, animal], tv=TruthValue(1.0, 0.99)
        )
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [animal, living], tv=TruthValue(1.0, 0.99)
        )
...
class TestPLNInduction(unittest.TestCase):
    ...
    def test_simple_induction(self):
        ...
        # Tweety is a bird
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [tweety, bird], tv=TruthValue(1.0, 0.99)
        )
        # Tweety flies
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [tweety, flies], tv=TruthValue(1.0, 0.99)
        )
    ...
    def test_multiple_instance_induction(self):
        ...
        for i in range(5):
            ...
            self.atomspace.add_link(
                AtomType.INHERITANCE_LINK, [instance, bird], tv=TruthValue(1.0, 0.99)
            )
            self.atomspace.add_link(
                AtomType.INHERITANCE_LINK, [instance, flies], tv=TruthValue(1.0, 0.99)
            )
...
class TestPLNAbduction(unittest.TestCase):
    ...
    def test_simple_abduction(self):
        ...
        self.atomspace.add_link(
            AtomType.IMPLICATION_LINK, [rain, wet_grass], tv=TruthValue(0.9, 0.9)
        )
        # Sprinkler causes wet grass
        self.atomspace.add_link(
            AtomType.IMPLICATION_LINK, [sprinkler, wet_grass], tv=TruthValue(0.95, 0.9)
        )
...
class TestPLNModusPonens(unittest.TestCase):
    ...
    def test_modus_ponens(self):
        ...
        # P -> Q (implication)
        self.atomspace.add_link(
            AtomType.IMPLICATION_LINK, [p, q], tv=TruthValue(0.95, 0.9)
        )
...
class TestPLNRevision(unittest.TestCase):
    ...
    def test_revision_increases_confidence(self):
        ...
        concept = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "TestConcept", tv=TruthValue(0.8, 0.5)
        )
...
class TestPLNForwardChaining(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        config = PLNConfig(max_depth=15, confidence_threshold=0.1)
        self.pln = PLNEngine(self.atomspace, config)
    ...
    def test_forward_chain_discovers_new_knowledge(self):
        ...
        # Socrates is human
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [socrates, human], tv=TruthValue(1.0, 0.99)
        )
        # Socrates is Greek
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [socrates, greek], tv=TruthValue(1.0, 0.99)
        )
        # Socrates is a philosopher
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [socrates, philosopher], tv=TruthValue(1.0, 0.99)
        )
...
class TestPLNAttentionGuided(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        config = PLNConfig(confidence_threshold=0.1)
        self.pln = PLNEngine(self.atomspace, config)
    ...
    def test_high_attention_atoms_preferred(self):
        ...
        high_att = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "HighAttention", av=AttentionValue(sti=0.9, lti=0.5)
        )
        low_att = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "LowAttention", av=AttentionValue(sti=0.1, lti=0.5)
        )
        target = self.atomspace.add_node(AtomType.CONCEPT_NODE, "Target")

        # Both link to target
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [high_att, target], tv=TruthValue(0.8, 0.8)
        )
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [low_att, target], tv=TruthValue(0.8, 0.8)
        )
