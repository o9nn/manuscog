...
class TestECANStimulation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        params = ECANParameters(
            total_stimulus=1000.0, focus_boundary=0.5, rent_rate=0.01, wage_rate=0.1
        )
    ...
    def test_stimulate_increases_sti(self):
        ...
        atom = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "TestAtom", av=AttentionValue(sti=0.3, lti=0.5)
        )
    ...
    def test_stimulate_multiple_atoms(self):
        ...
        for i in range(10):
            handle = self.atomspace.add_node(
                AtomType.CONCEPT_NODE, f"Atom_{i}", av=AttentionValue(sti=0.1, lti=0.5)
            )
            atoms.append(handle)
    ...
    def test_stimulate_respects_budget(self):
        ...
        for i in range(100):
            handle = self.atomspace.add_node(
                AtomType.CONCEPT_NODE, f"Atom_{i}", av=AttentionValue(sti=0.0, lti=0.5)
            )
            atoms.append(handle)
        ...
        total_sti = sum(self.atomspace.get_atom(a).attention_value.sti for a in atoms)
...
class TestECANAttentionalFocus(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        params = ECANParameters(focus_boundary=0.5, max_focus_size=20)
        self.ecan = ECANService(self.atomspace, params)
    ...
    def test_focus_size_limit(self):
        ...
        for i in range(50):
            self.atomspace.add_node(
                AtomType.CONCEPT_NODE, f"Atom_{i}", av=AttentionValue(sti=0.9, lti=0.5)
            )
    ...
    def test_focus_updates_dynamically(self):
        ...
        atom1 = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Atom1", av=AttentionValue(sti=0.9, lti=0.5)
        )
        atom2 = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Atom2", av=AttentionValue(sti=0.1, lti=0.5)
        )

        # Initially atom1 should be in focus (if any)
        self.ecan.get_attentional_focus()
...
class TestECANHebbianLinks(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        params = ECANParameters(hebbian_strength=0.5)
        self.ecan = ECANService(self.atomspace, params)

    def test_hebbian_link_creation(self):
        """Test that Hebbian links are created between co-active atoms."""
        atom1 = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Atom1", av=AttentionValue(sti=0.8, lti=0.5)
        )
        atom2 = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Atom2", av=AttentionValue(sti=0.8, lti=0.5)
        )
...
class TestECANImportanceDiffusion(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        params = ECANParameters(spread_threshold=0.3, spread_decay=0.5)
        self.ecan = ECANService(self.atomspace, params)

    def test_importance_spreads_through_links(self):
        """Test that importance spreads through links."""
        source = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Source", av=AttentionValue(sti=0.9, lti=0.5)
        )
        target = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Target", av=AttentionValue(sti=0.1, lti=0.5)
        )

        # Create link between them
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [source, target], tv=TruthValue(0.9, 0.9)
        )

        self.atomspace.get_atom(target).attention_value.sti
    ...
    def test_diffusion_respects_link_weight(self):
        ...
        strong_target = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "StrongTarget", av=AttentionValue(sti=0.1, lti=0.5)
        )
        weak_target = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "WeakTarget", av=AttentionValue(sti=0.1, lti=0.5)
        )

        # Strong link
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [source, strong_target], tv=TruthValue(0.95, 0.9)
        )
        # Weak link
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [source, weak_target], tv=TruthValue(0.3, 0.9)
        )
...
class TestECANRentAndWages(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        params = ECANParameters(rent_rate=0.05, wage_rate=0.1)
        self.ecan = ECANService(self.atomspace, params)

    def test_rent_decreases_sti(self):
        """Test that rent decreases STI over time."""
        atom = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "RentTest", av=AttentionValue(sti=0.5, lti=0.5)
        )
    ...
    def test_useful_atoms_earn_wages(self):
        ...
        useful = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "UsefulAtom", av=AttentionValue(sti=0.3, lti=0.5)
        )
...
class TestECANStressTest(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        params = ECANParameters(total_stimulus=10000.0, max_focus_size=100)
        self.ecan = ECANService(self.atomspace, params)
