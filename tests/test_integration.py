...
from kernel.attention.ecan import ECANParameters, ECANService
...
from kernel.reasoning.pln import PLNConfig, PLNEngine
...
class TestPLNECANIntegration(unittest.TestCase):
    ...
    def test_inference_boosts_attention(self):
        ...
        a = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "A", av=AttentionValue(sti=0.3, lti=0.5)
        )
        b = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "B", av=AttentionValue(sti=0.3, lti=0.5)
        )
        c = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "C", av=AttentionValue(sti=0.3, lti=0.5)
        )

        # Create inference chain
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [a, b], tv=TruthValue(0.9, 0.9)
        )
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [b, c], tv=TruthValue(0.9, 0.9)
        )
    ...
    def test_attention_guides_inference(self):
        ...
        high_a = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "HighA", av=AttentionValue(sti=0.9, lti=0.5)
        )
        high_b = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "HighB", av=AttentionValue(sti=0.9, lti=0.5)
        )

        # Create low-attention path
        low_a = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "LowA", av=AttentionValue(sti=0.1, lti=0.5)
        )
        low_b = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "LowB", av=AttentionValue(sti=0.1, lti=0.5)
        )

        # Links
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [high_a, high_b], tv=TruthValue(0.9, 0.9)
        )
        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [low_a, low_b], tv=TruthValue(0.9, 0.9)
        )
    ...
    def test_inference_cycle_with_attention(self):
        ...
        human = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Human", av=AttentionValue(sti=0.5, lti=0.5)
        )
        mortal = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Mortal", av=AttentionValue(sti=0.3, lti=0.5)
        )

        self.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [human, mortal], tv=TruthValue(1.0, 0.95)
        )
...
class TestMOSESAtomSpaceIntegration(unittest.TestCase):
    ...
    def test_learning_uses_existing_knowledge(self):
        ...
        self.atomspace.add_node(AtomType.SCHEMA_NODE, "double")
...
class TestPatternReasoningIntegration(unittest.TestCase):
    ...
    def test_patterns_inform_reasoning(self):
        ...
        for i in range(10):
            ...
            self.atomspace.add_link(
                AtomType.INHERITANCE_LINK, [cat, animal], tv=TruthValue(1.0, 0.9)
            )
...
class TestCognitiveCycleIntegration(unittest.TestCase):
    ...
    def test_full_cognitive_cycle(self):
        ...
        cat = self.kernel.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Cat", av=AttentionValue(sti=0.7, lti=0.5)
        )
        animal = self.kernel.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Animal", av=AttentionValue(sti=0.5, lti=0.5)
        )
        self.kernel.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [cat, animal], tv=TruthValue(1.0, 0.95)
        )
    ...
    def test_goal_driven_cognition(self):
        ...
        self.kernel.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [cat, mammal], tv=TruthValue(1.0, 0.9)
        )
...
class TestMemoryManagementIntegration(unittest.TestCase):
    ...
    def setUp(self):
        self.atomspace = AtomSpace()
        config = MemoryConfig(
            max_atoms=1000, forgetting_threshold=0.1, consolidation_interval=1.0
        )
    ...
    def test_forgetting_low_attention_atoms(self):
        ...
        high_att = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "HighAtt", av=AttentionValue(sti=0.9, lti=0.8)
        )
        low_att = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "LowAtt", av=AttentionValue(sti=0.01, lti=0.01)
        )
    ...
    def test_consolidation_preserves_important(self):
        ...
        important = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, "Important", av=AttentionValue(sti=0.8, lti=0.9)
        )
...
class TestSubsystemCoordination(unittest.TestCase):
    ...
    def test_service_communication(self):
        ...
        concept = self.kernel.atomspace.add_node(
            AtomType.CONCEPT_NODE, "TestConcept", av=AttentionValue(sti=0.5, lti=0.5)
        )
...
class TestEmergentBehavior(unittest.TestCase):
    ...
    def test_knowledge_growth(self):
        ...
        self.kernel.atomspace.add_link(
            AtomType.INHERITANCE_LINK, [mammal, animal], tv=TruthValue(1.0, 0.9)
        )
    ...
    def test_attention_dynamics(self):
        ...
        for _ in range(10):
            ...
            total_sti = sum(
                self.kernel.atomspace.get_atom(a).attention_value.sti for a in atoms
            )
