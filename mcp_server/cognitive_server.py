...
class CognitiveMCPServer:
    ...
    async def _execute_learn(self, args: Dict[str, Any]) -> MCPToolResult:
        ...
        if action == "add_concept":
            ...
            if not related:
                return MCPToolResult(
                    content=[
                        {"type": "text", "text": "Error: related_concept required"}
                    ],
                    isError=True,
                )
            ...
            return MCPToolResult(
                content=[
                    {"type": "text", "text": f"Strengthened belief about '{concept}'"}
                ]
            )
                    ...
                    "text": json.dumps(
                        {
                            ...
                            "total_pln_inferences": self.kernel.pln.stats.get(
                                "total_inferences", 0
                            ),
                        },
                        indent=2,
    ...
    async def _execute_focus(self, args: Dict[str, Any]) -> MCPToolResult:
        ...
        if action == "focus":
            if not concept:
                return MCPToolResult(
                    content=[
                        {"type": "text", "text": "Error: concept required for focus"}
                    ],
                    isError=True,
                )
            ...
            if not handles:
                return MCPToolResult(
                    content=[
                        {"type": "text", "text": f"Concept '{concept}' not found"}
                    ],
                    isError=True,
                )
            ...
            if self.kernel.ecan:
                ...
                return MCPToolResult(
                    content=[
                        {"type": "text", "text": "Attention decayed across all atoms"}
                    ]
                )
                            ...
                            "text": json.dumps(
                                {
                                    ...
                                    "top_focus": [
                                        {
                                            "name": getattr(a, "name", "Link"),
                                            "sti": a.attention_value.sti,
                                        }
                                        for a in focus
                                    ],
                                },
    ...
    async def _execute_reflect(self, args: Dict[str, Any]) -> MCPToolResult:
        ...
        args.get("apply_optimizations", False)
