...
from kernel.autognosis.orchestrator import AutognosisConfig, AutognosisOrchestrator
...
async def demo_autognosis_optimizer():
    ...
    try:
        ...
        initial_params = {
            "pln_inference_depth": kernel.pln.max_inference_depth
            if kernel.pln
            else None,
            "ecan_decay_rate": kernel.ecan.params.sti_decay_rate
            if kernel.ecan
            else None,
            "ecan_spread_decay": kernel.ecan.params.spread_decay
            if kernel.ecan
            else None,
            ...
            if result.optimizations:
                ...
                for opt_result in opt_results:
                    if opt_result.success:
                        ...
                        DemoOutput.data(
                            "  Est. improvement",
                            f"{opt_result.improvement_estimate:.1%}",
                        )
