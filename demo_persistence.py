...
def demo_persistence():
    ...
    try:
        ...
        for name, category in concepts:
            ...
            if not cat_handles:
                kernel1.atomspace.add_node(
                    AtomType.CONCEPT_NODE, category, tv=TruthValue(1.0, 0.9)
                )
        ...
        DemoOutput.success(
            f"Added {len(concepts)} concepts and {len(relationships)} relationships"
        )
        ...
        if kernel1.pln:
            ...
            DemoOutput.success(
                f"Made {kernel1.pln.stats['total_inferences']} inferences"
            )
        ...
        persistence_config = PersistenceConfig(
            backend=StorageBackend.SQLITE, storage_path=db_path
        )
        ...
        for name, _ in concepts:
            ...
            if handles:
                ...
                if atom:
                    DemoOutput.success(
                        f"Found '{name}' (strength: {atom.truth_value.strength:.2f})"
                    )

        DemoOutput.data(
            "Concepts preserved", f"{len(preserved_concepts)}/{len(concepts)}"
        )
        ...
        DemoOutput.data(
            "Relationships preserved", f"{preserved_links}/{len(relationships)}"
        )
        ...
        if kernel2.pln:
            ...
            DemoOutput.success(
                f"Made {kernel2.pln.stats['total_inferences']} new inferences"
            )
