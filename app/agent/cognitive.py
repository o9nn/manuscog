...
class CognitiveQueryTool:
    ...
    def _query_concept(self, concept: str) -> str:
        ...
        for atom in atoms:
            results.append(
                f"- {atom.name} (type: {atom.atom_type.name}, "
...
class CognitiveLearnTool:
    ...
    def _add_relationship(
        self, child: str, parent: str, relation_type: str, strength: float
    ) -> str:
        ...
        link_handle = self.kernel.atomspace.add_link(
            link_type, [child_handle, parent_handle], tv=TruthValue(strength, 0.8)
        )
    ...
    def _strengthen_belief(self, concept1: str, concept2: str, strength: float) -> str:
        ...
        existing = self.kernel.atomspace.match_pattern(
            AtomType.INHERITANCE_LINK, [c1_handle, c2_handle]
        )
...
class CognitiveAttentionTool:
    ...
    async def execute(
        self, action: str, concept: str = None, amount: float = 0.5
    ) -> str:
...
class CognitiveAgent(ToolCallAgent):
    ...
    kernel_config: KernelConfig = Field(
        default_factory=lambda: KernelConfig(
            ...
            for i, atom1 in enumerate(focus):
                for atom2 in focus[i + 1 :]:
                    self.kernel.hebbian.learn_association(
                        atom1.handle, atom2.handle, strength=0.5
                    )
