...
class DemoOutput:
    ...
    def header(cls, text: str):
        ...
        print(
            f"{cls.COLORS['bold']}{cls.COLORS['header']}{text.center(60)}{cls.COLORS['end']}"
        )
        print(
            f"{cls.COLORS['bold']}{cls.COLORS['header']}{'='*60}{cls.COLORS['end']}\n"
        )
...
class CognitiveThinkingDemo:
    ...
    async def _phase_attention_dynamics(self):
        ...
        ai_handles = self.kernel.atomspace._name_index.get_by_name(
        ...
        if ai_handles:
            ...
            if ai_atom:
                DemoOutput.attention(
                    f"Stimulating attention on 'Artificial Intelligence'..."
                )
        ...
        DemoOutput.data(
            "  Stimulus distributed", f"{ecan_stats.get('stimulus_distributed', 0):.2f}"
        )
        DemoOutput.data(
            "  Rent collected", f"{ecan_stats.get('rent_collected', 0):.2f}"
        )
    ...
    async def _phase_autognosis(self):
        ...
        DemoOutput.info(
            "Building hierarchical self-images at multiple cognitive levels.\n"
        )
        ...
        if not self.kernel.autognosis:
            ...
            DemoOutput.thinking(
                "Level 2 (Meta-cognitive): Evaluating self-awareness quality..."
            )
            DemoOutput.thinking(
                "Level 3 (Existential): Reflecting on purpose and identity..."
            )

            DemoOutput.insight(
                "Self-awareness assessment: The system is functioning normally"
            )
            DemoOutput.insight(
                "Optimization opportunity: Could increase inference depth"
            )
            return
        ...
        try:
            ...
            DemoOutput.data(
                "Self-awareness score", f"{result.self_awareness_score:.2f}"
            )
            ...
            for level, image in result.self_images.items():
                ...
                DemoOutput.thinking(
                    f"Level {level}: {level_desc} (confidence: {image.confidence:.2f})"
                )
            ...
            if result.insights:
                ...
                for insight in result.insights[:3]:
                    DemoOutput.insight(
                        f"{insight.title}: {insight.description[:60]}..."
                    )
            ...
            if result.optimizations:
                ...
                for opt in result.optimizations[:2]:
                    DemoOutput.info(
                        f"  - {opt.title} (expected improvement: {opt.expected_improvement:.0%})"
                    )
    ...
    async def _phase_demonstrate_learning(self):
        ...
        if python_handles and ai_handles:
            ...
            DemoOutput.thinking(
                "Added uncertain belief: Python → AI (strength=0.7, confidence=0.5)"
            )

            # Add more evidence
            if new_link:
                self.kernel.atomspace.merge_truth_value(
                    new_link, TruthValue(0.8, 0.6)  # More evidence
                )
