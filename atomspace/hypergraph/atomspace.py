...
class AttentionIndex:
    ...
    def update(self, handle: AtomHandle, av: AttentionValue):
        with self._lock:
            ...
            self._sti_sorted = [(sti, h) for sti, h in self._sti_sorted if h != handle]
            # Add new entry
...
class AtomSpaceEventBus:
    ...
    def __init__(self, buffer_size: int = 10000):
        self._subscribers: Dict[
            str, List[Callable[[AtomSpaceEvent], None]]
        ] = defaultdict(list)
...
class AtomSpace:
    ...
    def __init__(
        self, name: str = "default", node_id: str = "local", cache_size: int = 10000
    ):
        self.name = name
    ...
    def add_node(
        ...
        with self._lock:
            ...
            self._event_bus.emit(
                AtomSpaceEvent(event_type="add", handle=node.handle, new_value=node)
            )
    ...
    def add_link(
        ...
        with self._lock:
            ...
            # Emit event
            self._event_bus.emit(
                AtomSpaceEvent(event_type="add", handle=link.handle, new_value=link)
            )
    ...
    def remove_atom(self, handle: AtomHandle, recursive: bool = False) -> bool:
        ...
        with self._lock:
            ...
            # Emit event
            self._event_bus.emit(
                AtomSpaceEvent(event_type="remove", handle=handle, old_value=atom)
            )
    ...
    def get_atoms_by_type(
        self, atom_type: AtomType, subtype: bool = False
    ) -> List[Atom]:
    ...
    def get_neighbors(
        self, handle: AtomHandle, link_type: Optional[AtomType] = None
    ) -> List[Atom]:
    ...
    def get_top_attention(self, n: int = 10) -> List[Tuple[float, Atom]]:
        ...
        return [(sti, self._atoms[h]) for sti, h in top if h in self._atoms]
    ...
    def spread_attention(
        self, source: AtomHandle, spread_factor: float = 0.1, max_spread: int = 10
    ) -> int:
...
def create_inheritance(
    ...
    return atomspace.add_link(
        AtomType.INHERITANCE_LINK, [child, parent], tv=tv or TruthValue(1.0, 0.9)
    )
...
def create_similarity(
    ...
    return atomspace.add_link(
        AtomType.SIMILARITY_LINK, [atom1, atom2], tv=tv or TruthValue(0.5, 0.5)
    )
...
def create_evaluation(
    ...
    if list_link:
        return atomspace.add_link(
            AtomType.EVALUATION_LINK, [predicate, list_link], tv=tv
        )
    return None
