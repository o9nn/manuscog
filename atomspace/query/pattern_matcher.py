...
from kernel.cognitive.types import Atom, AtomHandle, AtomType, Link, Node, TruthValue
...
class VariableNode(Node):
    ...
    def __init__(
        ...
        super().__init__(atom_type=AtomType.VARIABLE_NODE, name=name)
...
class UnificationEngine:
    ...
    def unify(
        self, pattern: Any, atom: Atom, bindings: Dict[str, Atom] = None
    ...
    def _unify_variable(
        self, var: VariableNode, atom: Atom, bindings: Dict[str, Atom]
    ) -> Optional[Dict[str, Atom]]:
    ...
    def _unify_pattern_link(
        self, pattern: PatternLink, atom: Atom, bindings: Dict[str, Atom]
    ) -> Optional[Dict[str, Atom]]:
...
class PatternMatcher:
    ...
    def __init__(self, atomspace: AtomSpace):
        ...
        # Statistics
        self.stats = {"queries": 0, "matches_found": 0, "atoms_examined": 0}

    def match(self, pattern: QueryPattern, limit: int = 100) -> List[MatchResult]:
        """
        ...
        for link in self.atomspace.get_atoms_by_type(atom_type):
            ...
            if outgoing_pattern is None:
                ...
                for i, pattern_handle in enumerate(outgoing_pattern):
                    if (
                        pattern_handle is not None
                        and link.outgoing[i] != pattern_handle
                    ):
                        match = False
                        break
                if match:
    ...
    def find_by_predicate(
        ...
        for link in self.atomspace.get_incoming(predicate.handle):
            ...
            # Get argument atoms
            arguments = [self.atomspace.get_atom(h) for h in list_link.outgoing]
    ...
    def find_inheritance_chain(
        self, start: AtomHandle, end: AtomHandle, max_depth: int = 10
    ) -> Optional[List[Link]]:
        """
    ...
    def find_similar_atoms(
        self, atom: Atom, threshold: float = 0.5, limit: int = 10
...
class QueryBuilder:
    ...
    def var(
        self, name: str, type_constraint: Optional[AtomType] = None
    ) -> VariableNode:
    ...
    def inheritance(self, child: Any, parent: Any) -> "QueryBuilder":
        """Set pattern to inheritance link."""
        self._pattern = PatternLink(
            atom_type=AtomType.INHERITANCE_LINK, outgoing=[child, parent]
        )
        return self

    def similarity(self, atom1: Any, atom2: Any) -> "QueryBuilder":
        """Set pattern to similarity link."""
        self._pattern = PatternLink(
            atom_type=AtomType.SIMILARITY_LINK, outgoing=[atom1, atom2]
        )
        return self

    def evaluation(self, predicate: Any, arguments: List[Any]) -> "QueryBuilder":
        """Set pattern to evaluation link."""
        list_pattern = PatternLink(atom_type=AtomType.LIST_LINK, outgoing=arguments)
        self._pattern = PatternLink(
            atom_type=AtomType.EVALUATION_LINK, outgoing=[predicate, list_pattern]
        )
        return self

    def where(self, constraint: Callable[[Dict[str, Atom]], bool]) -> "QueryBuilder":
        """Add a constraint."""
...
class GraphPatternMatcher:
    ...
    def match_subgraph(
        self, pattern_atoms: List[Atom], pattern_links: List[PatternLink]
    ) -> List[Dict[str, Atom]]:
        """
        ...
        first_matches = self.pattern_matcher.match(QueryPattern.create(first_link))
