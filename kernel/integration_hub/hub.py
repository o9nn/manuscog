...
class ManusIntegrationHub:
    ...
    def __init__(self, config: HubConfig = None):
        ...
        self._connection_info = ConnectionInfo(
            protocol=self.config.primary_protocol, state=ConnectionState.DISCONNECTED
        )
    ...
    async def initialize(self, kernel: "CognitiveKernel"):
        ...
        self._primary_adapter = ProtocolAdapterFactory.create(
            self.config.primary_protocol, self._protocol_config
        )

        if self.config.enable_fallback:
            self._fallback_adapter = ProtocolAdapterFactory.create(
                self.config.fallback_protocol, self._protocol_config
            )
    ...
    async def connect(self) -> bool:
        ...
        if self.config.enable_fallback and self._fallback_adapter:
            ...
            if await self._fallback_adapter.connect():
                ...
                logger.info(
                    f"Connected via fallback {self.config.fallback_protocol.name}"
                )
                return True
    ...
    async def request_guidance(
        ...
        request = GuidanceRequest(
            topic=topic, question=question, context=context or {}, urgency=urgency
        )

        response = await self._send_request(
            request.to_message(), RequestType.GUIDANCE_REQUEST
        )
    ...
    async def request_analysis(
        ...
        response = await self._send_request(
            request.to_message(), RequestType.ANALYSIS_REQUEST
        )
    ...
    async def propose_evolution(
        ...
        response = await self._send_request(
            proposal.to_message(), RequestType.EVOLUTION_PROPOSAL
        )
    ...
    async def query_knowledge(
        ...
        """
        request = KnowledgeQuery(query=query, domain=domain, max_results=max_results)

        response = await self._send_request(
            request.to_message(), RequestType.KNOWLEDGE_QUERY
        )
    ...
    async def send_status_report(self) -> bool:
        ...
        report = StatusReport(
            kernel_state=self._kernel.state.name
            if hasattr(self._kernel, "state")
            else "unknown",
    ...
    async def _send_request(
        self, message: IntegrationMessage, request_type: RequestType
        ...
        try:
            response = await asyncio.wait_for(
                response_future, timeout=self.config.request_timeout
            )
    ...
    async def _handle_incoming_message(self, message: IntegrationMessage):
        ...
        if message_type == ResponseType.TASK_ASSIGNMENT.value:
            await self._handle_task_assignment(
                TaskAssignment.from_payload(message.payload)
            )
            ...
            await self._handle_knowledge_injection(
                KnowledgeInjection.from_payload(message.payload)
            )
