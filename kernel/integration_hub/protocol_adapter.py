...
from .types import ConnectionInfo, ConnectionState, IntegrationMessage, ProtocolType
...
class BaseProtocolAdapter(ABC):
    ...
    def __init__(self, config: ProtocolConfig):
        ...
        self.connection_info = ConnectionInfo(
            protocol=self.protocol_type, state=ConnectionState.DISCONNECTED
        )
        self._message_handlers: List[
            Callable[[IntegrationMessage], Awaitable[None]]
        ] = []
    ...
    def add_message_handler(
        self, handler: Callable[[IntegrationMessage], Awaitable[None]]
    ):
    ...
    async def send(self, message: IntegrationMessage) -> bool:
        ...
        try:
            async with self._session.post(
                f"{self.config.rest_base_url}/messages", json=message.to_dict()
            ) as resp:
                self.connection_info.messages_sent += 1
                self.connection_info.last_heartbeat = datetime.now()
