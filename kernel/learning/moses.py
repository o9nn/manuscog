    ...
    def evaluate(self, inputs: Dict[str, Any]) -> Any:
        ...
        if self.node_type == ProgramNodeType.CONSTANT:
            ...
            if len(self.children) >= 2:
                return self.children[0].evaluate(inputs) - self.children[1].evaluate(
                    inputs
                )
            return 0
            ...
            if len(self.children) >= 3:
                ...
                return (
                    self.children[1].evaluate(inputs)
                    if cond
                    else self.children[2].evaluate(inputs)
                )
            return 0
        return 0
...
class Program:
    ...
    program_id: str = field(
        default_factory=lambda: f"prog_{random.randint(0, 99999):05d}"
    )
...
class MutationOperator:
    ...
    def __init__(
        self, input_vars: List[str], constant_range: Tuple[float, float] = (-10, 10)
    ):
    ...
    def _mutate_node(self, node: ProgramNode, rate: float):
        ...
        if random.random() < rate:
            ...
            mutation_type = random.choice(
                ["change_op", "change_value", "grow", "shrink"]
            )

            if mutation_type == "change_op" and node.node_type not in [
                ProgramNodeType.CONSTANT,
                ProgramNodeType.INPUT,
            ]:
                # Change operator
    ...
    def _get_similar_operators(self, op: ProgramNodeType) -> List[ProgramNodeType]:
        ...
        arithmetic = [
            ProgramNodeType.PLUS,
            ProgramNodeType.MINUS,
            ProgramNodeType.TIMES,
            ProgramNodeType.DIVIDE,
        ]
        comparison = [
            ProgramNodeType.GREATER,
            ProgramNodeType.LESS,
            ProgramNodeType.EQUAL,
        ]
    ...
    def _random_terminal(self) -> ProgramNode:
        ...
        if random.random() < 0.5:
            ...
            return ProgramNode(
                node_type=ProgramNodeType.INPUT, value=random.choice(self.input_vars)
            )
...
class CrossoverOperator:
    ...
    def _collect_nodes(
        self, node: ProgramNode, parent: ProgramNode = None, index: int = 0
    ) -> List[Dict]:
    ...
    def select_parents(self, n: int = 2, tournament_size: int = 3) -> List[Program]:
        ...
        for _ in range(n):
            tournament = random.sample(
                self.programs, min(tournament_size, len(self.programs))
            )
...
class DemeManager:
    ...
    def __init__(
        self, num_demes: int = 4, deme_size: int = 50, migration_rate: float = 0.1
    ):
...
class MOSESEngine(LearningService):
    ...
    def _random_program(
        self, max_depth: int = 4, current_depth: int = 0
    ) -> ProgramNode:
        ...
        children = [
            self._random_program(max_depth, current_depth + 1) for _ in range(arity)
        ]
...
class HebbianLearner:
    ...
    def learn_association(
        self, atom1: AtomHandle, atom2: AtomHandle, strength: float = 1.0
    ):
        """Strengthen association between two atoms."""
        # Find or create Hebbian link
        existing = self.atomspace.match_pattern(AtomType.HEBBIAN_LINK, [atom1, atom2])

        if existing:
            # Strengthen existing link
            link = existing[0]
            new_strength = min(
                1.0, link.truth_value.strength + self.learning_rate * strength
            )
            new_tv = TruthValue(
                new_strength, link.truth_value.confidence, link.truth_value.count + 1
            )
    ...
    def decay_associations(self, decay_rate: float = 0.99):
        ...
        for link in self.atomspace.get_atoms_by_type(AtomType.HEBBIAN_LINK):
            ...
            if new_strength < 0.01:
                ...
                new_tv = TruthValue(
                    new_strength, link.truth_value.confidence, link.truth_value.count
                )
                self.atomspace.set_truth_value(link.handle, new_tv)
