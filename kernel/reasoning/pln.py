...
class PLNFormulas:
    ...
    def deduction(
        ...
        tv_b.strength if tv_b else PLNFormulas.DEFAULT_PRIOR
    ...
    def abduction(
        ...
        # Get priors
        tv_a.strength if tv_a else PLNFormulas.DEFAULT_PRIOR
        sB = tv_b.strength if tv_b else PLNFormulas.DEFAULT_PRIOR
        sC = tv_c.strength if tv_c else PLNFormulas.DEFAULT_PRIOR
    ...
    @staticmethod
    def modus_ponens(tv_a: TruthValue, tv_ab: TruthValue) -> TruthValue:  # A  # A -> B
        """
        Modus Ponens: A, A->B => B
    ...
    def negation(tv: TruthValue) -> TruthValue:
        """Negation: NOT A"""
        return TruthValue(1.0 - tv.strength, tv.confidence, tv.count)
...
class InferenceRuleSet:
    ...
    def get_applicable_rules(self, input_types: List[AtomType]) -> List[InferenceRule]:
    ...
    def _types_match(
        self, rule_types: List[AtomType], input_types: List[AtomType]
    ) -> bool:
...
class InferenceController:
    ...
    def find_deduction_premises(self) -> List[Tuple[Link, Link]]:
        ...
        for link_ab in inheritance_links:
            ...
            for link_bc in self.atomspace.get_incoming(b_handle):
                if (
                    isinstance(link_bc, Link)
                    and link_bc.atom_type == AtomType.INHERITANCE_LINK
                    and len(link_bc.outgoing) >= 2
                    and link_bc.outgoing[0] == b_handle
                ):
    ...
    def select_premises_by_attention(
        self, candidates: List[Tuple[Link, Link]], limit: int = 10
        ...
        for link1, link2 in candidates:
            score = (
                link1.attention_value.sti
                + link2.attention_value.sti
                + link1.truth_value.confidence
                + link2.truth_value.confidence
            )
...
class PLNEngine(ReasoningService):
    ...
    def __init__(
        ...
        if isinstance(config_or_depth, PLNConfig):
            ...
            self.max_inference_depth = (
                config_or_depth if config_or_depth is not None else 5
            )
    ...
    def start(self) -> bool:
        ...
        self._inference_thread = threading.Thread(
            target=self._inference_loop, daemon=True, name="pln-inference"
        )
    ...
    def infer(
        self, premises: List[AtomHandle], inference_type: InferenceType
    ...
    def deduction(self, link_ab: Link, link_bc: Link) -> Optional[InferenceResult]:
        """
        ...
        existing = self.atomspace.match_pattern(
            AtomType.INHERITANCE_LINK, [a_handle, c_handle]
        )
        ...
        if existing:
            ...
            new_handle = self.atomspace.add_link(
                AtomType.INHERITANCE_LINK, [a_handle, c_handle], tv=tv_ac
            )
            if new_handle:
    ...
    def induction(self, link_ab: Link, link_ac: Link) -> Optional[InferenceResult]:
        """
        ...
        existing = self.atomspace.match_pattern(
            AtomType.INHERITANCE_LINK, [b_handle, c_handle]
        )
        ...
        if existing:
            ...
            new_handle = self.atomspace.add_link(
                AtomType.INHERITANCE_LINK, [b_handle, c_handle], tv=tv_bc
            )
            if new_handle:
                self.stats["new_atoms_created"] += 1
                conclusion = self.atomspace.get_atom(new_handle)
    ...
    def abduction(self, link_ab: Link, link_cb: Link) -> Optional[InferenceResult]:
        """
    ...
    def modus_ponens(self, atom_a: Atom, link_ab: Link) -> Optional[InferenceResult]:
        """
    ...
    def forward_chain(
        self, max_steps: int = 100, focus_atoms: Optional[Set[AtomHandle]] = None
        ...
        while steps < max_steps:
            ...
            if focus_atoms:
                deduction_premises = [
                    (l1, l2)
                    for l1, l2 in deduction_premises
            ...
            if steps < max_steps:
                ...
                if focus_atoms:
                    induction_premises = [
                        (l1, l2)
                        for l1, l2 in induction_premises
                        if l1.handle in focus_atoms or l2.handle in focus_atoms
                    ]
    ...
    def backward_chain(
        self, goal: AtomHandle, max_depth: int = None
    ...
    def _backward_chain_recursive(
        self, goal: AtomHandle, depth: int, visited: Set[AtomHandle]
    ) -> Optional[List[InferenceResult]]:
                    ...
                    if sub_results is not None:
                        ...
                        if antecedent_atom:
                            ...
                            for ante_link in self.atomspace.get_incoming(antecedent):
                                if (
                                    ante_link.atom_type == AtomType.INHERITANCE_LINK
                                    and len(ante_link.outgoing) >= 2
                                    and ante_link.outgoing[1] == antecedent
                                ):
    ...
    def _apply_inference(
        self, atoms: List[Atom], rule: InferenceRule, inference_type: InferenceType
    ) -> Optional[InferenceResult]:
        """Apply an inference rule to atoms."""
        if inference_type == InferenceType.DEDUCTION:
            if (
                len(atoms) >= 2
                and isinstance(atoms[0], Link)
                and isinstance(atoms[1], Link)
            ):
