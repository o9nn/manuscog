...
# Attention
from kernel.attention.ecan import ECANService
...
# Emergent intelligence
from kernel.emergence.intelligence import EmergentIntelligenceService

# Learning
from kernel.learning.moses import HebbianLearner, MOSESEngine
...
# Process scheduler
from proc.scheduler.cognitive_scheduler import CognitiveScheduler, IPCManager
...
try:
    from kernel.autognosis import AutognosisConfig, AutognosisOrchestrator
...
try:
    from kernel.integration_hub import HubConfig, ManusIntegrationHub
...
try:
    from kernel.ontogenesis import EvolutionConfig, EvolutionEngine
...
# VORTEX architecture
try:
    from kernel.vortex import VortexConfig, VortexOrchestrator
...
class CognitiveKernel:
    ...
    def _init_memory_manager(self):
        ...
        mem_config = MemoryConfig(forgetting_threshold=self.config.forgetting_threshold)
    ...
    def _init_learning(self):
        ...
        self.moses = MOSESEngine(
            self.atomspace, population_size=self.config.population_size
        )
        self.hebbian = HebbianLearner(
            self.atomspace, learning_rate=self.config.learning_rate
        )
    ...
    def create_goal(
        self, name: str, description: str = "", priority: float = 0.5
    ) -> Optional[str]:
        ...
        goal = CognitiveGoal(name=name, description=description, priority=priority)
...
def create_kernel(
    kernel_id: str = None, enable_distribution: bool = False, **kwargs
) -> CognitiveKernel:
    """
...
if __name__ == "__main__":
    ...
    kernel = boot_kernel(kernel_id="main-kernel", log_level="INFO")
    ...
    if kernel.state == KernelState.RUNNING:
        ...
        for i in range(5):
            ...
            result = kernel.think()
            print(
                f"  Cycle {i+1}: Focus={result.get('focus_size', 0)}, "
