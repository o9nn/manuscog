...
def _partition_helper(
    n: int, max_val: int, current: List[int], result: List[List[int]]
):
...
def _generate_child_combinations(
    partition: List[int], trees_by_order: Dict[int, List[RootedTree]]
) -> List[Tuple[RootedTree, ...]]:
...
class BSeriesExpansion:
    ...
    def add_term(self, tree: RootedTree, coefficient: float, weight: float = 1.0):
        ...
        self.terms.append(
            BSeriesTerm(tree=tree, coefficient=coefficient, weight=weight)
        )
    ...
    def evaluate(self, h: float) -> float:
        ...
        for term in self.terms:
            contribution = (
                (h**term.tree.order / term.tree.symmetry())
                * term.coefficient
                * term.weight
            )
            result += contribution
...
class ButcherTableau:
    ...
    def euler(cls) -> "ButcherTableau":
        ...
        return cls(A=[[0.0]], b=[1.0], c=[0.0], name="Euler")
    ...
    def midpoint(cls) -> "ButcherTableau":
        ...
        return cls(
            A=[[0.0, 0.0], [0.5, 0.0]], b=[0.0, 1.0], c=[0.0, 0.5], name="Midpoint"
        )
...
def _compute_composed_coefficient(
    tree: RootedTree, b1: BSeriesExpansion, b2: BSeriesExpansion
) -> float:
