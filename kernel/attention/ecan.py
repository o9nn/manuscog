...
class ECANParameters:
    ...
    def aggressive(cls) -> "ECANParameters":
        ...
        return cls(
            rent_rate=0.05, sti_decay_rate=0.95, focus_boundary=0.6, max_focus_size=50
        )
...
class HebbianLinkManager:
    ...
    def __init__(self, atomspace: AtomSpace, params: ECANParameters):
        ...
        self._coactivation_counts: Dict[
            Tuple[AtomHandle, AtomHandle], int
        ] = defaultdict(int)
            ...
            for (h1, h2), count in list(self._coactivation_counts.items()):
                if count >= threshold:
                    ...
                    if existing:
                        ...
                        self.atomspace.add_link(AtomType.HEBBIAN_LINK, [h1, h2], tv=tv)

                    # Reset count
                    self._coactivation_counts[(h1, h2)] = 0

    def get_hebbian_neighbors(
        self, handle: AtomHandle, min_strength: float = 0.1
...
class AttentionalFocus:
    ...
    def update_focus(self) -> Set[AtomHandle]:
        ...
        with self._lock:
            ...
            top_atoms = self.atomspace.get_top_attention(self.params.max_focus_size * 2)
    ...
    def force_focus(self, handle: AtomHandle):
        ...
        with self._lock:
            ...
            if atom:
                ...
                new_sti = max(
                    atom.attention_value.sti, self.params.focus_boundary + 0.1
                )
...
class ECANService(AttentionService):
    ...
    def __init__(self, atomspace: AtomSpace, params: Optional[ECANParameters] = None):
        self.atomspace = atomspace
        ...
        self.diffusion = ImportanceDiffusion(
            atomspace, self.params, self.hebbian_manager
        )
    ...
    def start(self) -> bool:
        ...
        self._attention_thread = threading.Thread(
            target=self._attention_loop, daemon=True, name="ecan-attention"
        )
    ...
    def boost_importance(
        self, handle: AtomHandle, sti_boost: float = 0.0, lti_boost: float = 0.0
    ):
    ...
    def set_vlti(self, handle: AtomHandle, vlti: bool = True):
        ...
        new_av = AttentionValue(
            sti=atom.attention_value.sti, lti=atom.attention_value.lti, vlti=vlti
        )
        self.atomspace.set_attention_value(handle, new_av)
    ...
    def focus_on_goal(self, goal_handle: AtomHandle, spread: bool = True):
        ...
        if spread:
            self.diffusion.spread_importance(
                goal_handle, self.params.stimulus_per_atom * 5
            )
    ...
    def defocus(self, handle: AtomHandle):
        ...
        if atom:
            new_av = AttentionValue(
                sti=0.0, lti=atom.attention_value.lti, vlti=atom.attention_value.vlti
            )
            self.atomspace.set_attention_value(handle, new_av)
...
class AttentionBasedSelector:
    ...
    def select_by_attention(
        self, candidates: List[AtomHandle], n: int = 1, temperature: float = 1.0
    ) -> List[AtomHandle]:
        """
    ...
    def select_by_type_and_attention(
        self, atom_type: AtomType, n: int = 1
    ) -> List[AtomHandle]:
