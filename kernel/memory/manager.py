...
from kernel.cognitive.types import Atom, AtomHandle, AttentionValue, MemoryService
...
class ForgettingQueue:
    ...
    def __init__(self):
        self._heap: List[
            Tuple[float, float, AtomHandle]
        ] = []  # (importance, timestamp, handle)
...
class CognitiveMemoryManager(MemoryService):
    ...
    def __init__(
        ...
        self._on_consolidate_callbacks: List[
            Callable[[AtomHandle, MemoryRegion], None]
        ] = []
    ...
    def start(self) -> bool:
        ...
        self._decay_thread = threading.Thread(
            target=self._decay_loop, daemon=True, name="memory-decay"
        )
        self._decay_thread.start()

        self._consolidation_thread = threading.Thread(
            target=self._consolidation_loop, daemon=True, name="memory-consolidation"
        )
        self._consolidation_thread.start()

        self._forgetting_thread = threading.Thread(
            target=self._forgetting_loop, daemon=True, name="memory-forgetting"
        )
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "region_counts": {
                r.name: len(atoms) for r, atoms in self._region_atoms.items()
            },
        }
    ...
    def allocate_atom(self, atom: Atom) -> AtomHandle:
        ...
        with self._lock:
            ...
            allocation = MemoryAllocation(
                handle=atom.handle, region=region, size_bytes=self._estimate_size(atom)
            )
    ...
    def release_atom(self, handle: AtomHandle) -> bool:
        ...
        with self._lock:
            ...
            self._stats.bytes_by_region[allocation.region] = (
                self._stats.bytes_by_region.get(
                    allocation.region, allocation.size_bytes
                )
    ...
    def move_to_region(self, handle: AtomHandle, new_region: MemoryRegion) -> bool:
        ...
        with self._lock:
            ...
            self._stats.bytes_by_region[old_region] = (
                self._stats.bytes_by_region.get(old_region, allocation.size_bytes)
                - allocation.size_bytes
            )
