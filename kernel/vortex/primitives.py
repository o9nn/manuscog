...
class Vortex:
    ...
    def add_arm(self, direction: FlowDirection) -> VortexArm:
        ...
        arm = VortexArm(arm_id=arm_id, origin=self.core.position, direction=direction)
...
def compute_interaction(v1: Vortex, v2: Vortex) -> Optional[InteractionResult]:
    ...
    if distance < (v1.boundary.inner_radius + v2.boundary.inner_radius):
        ...
        coherence_diff < 0.1
        and abs(v1.state.angular_momentum - v2.state.angular_momentum) < 0.2
    ):
...
class VortexField:
    ...
    def update(self, delta_t: float) -> Dict[str, Any]:
        ...
        for vortex in self.vortices.values():
            inputs = {"energy": self.field_energy, "perturbation": 0.0}
    ...
    def _update_field_properties(self):
        ...
        self.field_coherence = sum(
            v.state.coherence for v in self.vortices.values()
        ) / len(self.vortices)
