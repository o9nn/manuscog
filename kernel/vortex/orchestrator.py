...
from .primitives import FlowDirection, Vortex, VortexField, VortexScale
...
class VortexOrchestrator:
    ...
    def _create_initial_vortices(self):
        ...
        main_vortex = self.field.create_vortex(
            scale=VortexScale.MACRO, position=(0.0, 0.0, 0.0)
        )
        ...
        for i in range(3):
            v = self.attention_field.create_vortex(
                scale=VortexScale.MESO, position=(float(i), 0.0, 0.0)
            )
    ...
    async def _update_all_fields(self):
        ...
        # Update main field
        self.field.update(delta_t)

        # Update specialized fields
        self.attention_field.update(delta_t)
        self.memory_field.update(delta_t)
        self.reasoning_field.update(delta_t)
