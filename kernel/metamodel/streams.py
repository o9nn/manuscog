...
class DynamicStream:
    ...
    def interact_with(
        self, other: "DynamicStream", interaction_type: str, strength: float
    ):
...
class EntropicStream(DynamicStream):
    ...
    def _update_internal(self, delta_t: float, inputs: Dict[str, float]):
        ...
        self.en_tropis = min(
            1.0, self.en_tropis + (external_energy + perturbation) * delta_t * 0.1
        )

        # Phase 2: auto-vortis (self-organizing vortex)
        # Emerges when en-tropis exceeds threshold
        if self.en_tropis > 0.3:
            self.auto_vortis = min(
                1.0, self.auto_vortis + self.en_tropis * delta_t * 0.05
            )
        ...
        if self.auto_vortis > 0.5:
            self.auto_morphosis = min(
                1.0, self.auto_morphosis + self.auto_vortis * delta_t * 0.03
            )
        ...
        # Update stream state
        self.state.energy = (
            self.en_tropis + self.auto_vortis + self.auto_morphosis
        ) / 3
        self.state.stability = (
            1.0 - self.vortex_intensity
        )  # Less stable when vortex is strong
        self.state.coherence = (
            self.auto_vortis
        )  # Coherence comes from vortex organization
...
class NegentropicStream(DynamicStream):
    ...
    def _update_internal(self, delta_t: float, inputs: Dict[str, float]):
        ...
        self.negen_tropis = max(
            0.0, min(1.0, self.negen_tropis + resistance * delta_t * 0.1)
        )
        ...
        if self.auto_stasis > 0.6 and resources > 0.5:
            self.auto_poiesis = min(
                1.0, self.auto_poiesis + self.auto_stasis * delta_t * 0.02
            )
...
class IdentityStream(DynamicStream):
    ...
    def _update_internal(self, delta_t: float, inputs: Dict[str, float]):
        ...
        if self.auto_gnosis > 0.6:
            ...
            self.auto_genesis = min(
                1.0, self.auto_genesis + genesis_potential * delta_t * 0.02
            )
        ...
        self.identity_coherence = (
            self.iden_tropis + self.auto_gnosis + self.auto_genesis
        ) / 3
...
class StreamTriad:
    ...
    def update(self, inputs: Dict[str, Dict[str, float]]) -> Dict[str, StreamState]:
        ...
        negentropic_state = self.negentropic.update(
            delta_t, inputs.get("negentropic", {})
        )
    ...
    def _process_interactions(self):
        ...
        if self.entropic.state.energy > 0.6:
            self.negentropic.negen_tropis = max(
                0.0,
                self.negentropic.negen_tropis - self.entropic.vortex_intensity * 0.1,
            )
            self.entropic.interact_with(
                self.negentropic, "balancing", self.entropic.vortex_intensity
            )

        # Negentropic → Identity: Reinforcing interaction
        # Stability enables identity formation
        if self.negentropic.auto_stasis > 0.5:
            self.identity.iden_tropis = min(
                1.0, self.identity.iden_tropis + self.negentropic.auto_stasis * 0.05
            )
            self.negentropic.interact_with(
                self.identity, "reinforcing", self.negentropic.auto_stasis
            )

        # Identity → Entropic: Transforming interaction
        # Self-knowledge enables directed transformation
        if self.identity.auto_gnosis > 0.5:
            self.entropic.transformation_potential = min(
                1.0,
                self.entropic.transformation_potential
                + self.identity.auto_gnosis * 0.1,
            )
            self.identity.interact_with(
                self.entropic, "transforming", self.identity.auto_gnosis
            )
