...
class DualState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"actual": self.actual, "virtual": self.virtual, "tension": self.tension}
...
class DualComplementarity:
    ...
    def get_dominant_pole(self) -> DualPole:
        ...
        return (
            DualPole.ACTUAL
            if self.state.actual > self.state.virtual
            else DualPole.VIRTUAL
        )
...
class TriadicSystem:
    ...
    def update(
        self, being_input: float, becoming_input: float, relation_input: float
    ) -> TriadicState:
