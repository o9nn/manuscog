...
class AutognosisOrchestrator:
    ...
    def __init__(self, config: AutognosisConfig = None):
        ...
        self.modeler = HierarchicalSelfModeler(
            ModelerConfig(max_levels=self.config.max_levels)
        )
        self.optimizer = AutognosisOptimizer(
            OptimizationPolicy(
    ...
    async def _generate_insights(self, observation) -> List[MetaCognitiveInsight]:
        ...
        if self.current_self_images:
            avg_confidence = sum(
                img.confidence for img in self.current_self_images.values()
            ) / len(self.current_self_images)
    ...
    async def _apply_safe_optimizations(
        self, kernel: "CognitiveKernel", optimizations: List[OptimizationOpportunity]
    ):
