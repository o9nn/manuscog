...
class SelfMonitor:
    ...
    def _observe_components(
        ...
        if kernel.atomspace:
            components["atomspace"] = ComponentState(
                ...
                metrics={
                    ...
                    "node_count": len(
                        [a for a in kernel.atomspace if hasattr(a, "name")]
                    ),
                    "link_count": len(
                        [a for a in kernel.atomspace if hasattr(a, "outgoing")]
                    ),
                },
            )
        ...
        if kernel.pln:
            components["pln"] = ComponentState(
                ...
                last_activity=datetime.now(),
                metrics={
                    "rules_available": len(kernel.pln.rules)
                    if hasattr(kernel.pln, "rules")
                    else 0
                },
            )
        ...
        if kernel.ecan:
            components["ecan"] = ComponentState(
                ...
                is_active=kernel.ecan.is_running
                if hasattr(kernel.ecan, "is_running")
                else True,
                health_score=1.0,
                load=0.0,
                last_activity=datetime.now(),
    ...
    def _measure_performance(self, kernel: "CognitiveKernel") -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            inferences_per_second=kernel.stats.get("inferences_made", 0)
            / max(1, kernel.stats.get("uptime", 1)),
    ...
    def _detect_patterns(self) -> List[BehavioralPattern]:
        ...
        if statistics.stdev(recent_cpu) < 0.05:
            pattern = self._get_or_create_pattern(
                "stable_cpu", "performance", "CPU utilization is stable"
            )
        ...
        if len(self._metrics_history) >= 20:
            ...
            if new_atoms > old_atoms * 1.1:
                pattern = self._get_or_create_pattern(
                    "growing_knowledge", "learning", "Knowledge base is growing"
                )
                pattern.last_observed = datetime.now()
                pattern.frequency = min(1.0, pattern.frequency + 0.1)
                patterns.append(pattern)
        ...
        recent_inferences = [
            m.inferences_per_second for m in self._metrics_history[-10:]
        ]
        avg_inferences = statistics.mean(recent_inferences)
        if avg_inferences > 10:
            pattern = self._get_or_create_pattern(
                "high_inference", "reasoning", "High inference activity detected"
            )
            pattern.last_observed = datetime.now()
            pattern.frequency = min(1.0, pattern.frequency + 0.1)
            patterns.append(pattern)
    ...
    def _get_or_create_pattern(
        self, pattern_id: str, pattern_type: str, description: str
    ) -> BehavioralPattern:
    ...
    def _update_baseline(self, metrics: PerformanceMetrics):
        ...
        for name, value in metrics_dict.items():
            ...
            if len(self._baseline_metrics[name]) > self.config.pattern_window_size:
                self._baseline_metrics[name] = self._baseline_metrics[name][
    ...
    def _estimate_atomspace_load(self, kernel: "CognitiveKernel") -> float:
        ...
        max_atoms = (
            kernel.config.max_atoms if hasattr(kernel.config, "max_atoms") else 1000000
        )
