...
class AutognosisOptimizer:
    ...
    async def _apply_optimization(
        ...
        if opportunity.optimization_type == OptimizationType.RESOURCE_REALLOCATION:
            ...
            logger.warning(
                f"Unknown optimization type: {opportunity.optimization_type}"
            )
            return None
    ...
    async def _optimize_resources(
        ...
        underutilized = any(
            i.insight_type == InsightType.RESOURCE_UTILIZATION
            and "underutilized" in i.description.lower()
            for i in insights
        )
        ...
        overutilized = any(
            i.insight_type == InsightType.RESOURCE_UTILIZATION
            and "overutilized" in i.description.lower()
            for i in insights
        )
    ...
    async def _tune_parameters(
        ...
        anomalies = [
            i for i in insights if i.insight_type == InsightType.ANOMALY_DETECTION
        ]
        ...
        if anomalies and kernel.ecan:
            ...
            # If attention is too flat, decrease decay
                ...
                "flat" in a.description.lower() or "uniform" in a.description.lower()
                for a in anomalies
            ):
    ...
    async def _consolidate_memory(
        ...
        for atom in kernel.atomspace:
            if (
                atom.attention_value.sti < -0.5
                and not atom.attention_value.vlti
                and atom.truth_value.confidence < 0.3
            ):
        ...
        if removed_count > 0:
            logger.info(
                f"Memory consolidation: removed {removed_count} low-importance atoms"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "success_rate": (
                self.stats["optimizations_successful"]
                / self.stats["optimizations_attempted"]
