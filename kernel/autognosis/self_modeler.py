...
from .types import BehavioralPattern, MetaReflection, SelfImage
...
class HierarchicalSelfModeler:
    ...
    async def build_self_image(
        self, level: int, monitor: SelfMonitor, kernel: "CognitiveKernel" = None
    ) -> SelfImage:
        """
        ...
        if level < 0 or level >= self.config.max_levels:
            raise ValueError(
                f"Level must be between 0 and {self.config.max_levels - 1}"
            )

        timestamp = datetime.now()

        if level == 0:
            image = await self._build_direct_observation_image(
                monitor, kernel, timestamp
            )
        elif level == 1:
    ...
    async def _build_direct_observation_image(
        self, monitor: SelfMonitor, kernel: "CognitiveKernel", timestamp: datetime
    ) -> SelfImage:
    ...
    async def _build_pattern_analysis_image(
        self, monitor: SelfMonitor, timestamp: datetime
    ) -> SelfImage:
    ...
    async def _build_metacognitive_image(
        self, level: int, monitor: SelfMonitor, timestamp: datetime
    ) -> SelfImage:
    ...
    def _generate_pattern_reflections(
        self, patterns: List[BehavioralPattern], timestamp: datetime
    ) -> List[MetaReflection]:
    ...
    def _generate_meta_reflections(
        self, lower_image: SelfImage, level: int, timestamp: datetime
    ) -> List[MetaReflection]:
        ...
        if lower_image.meta_reflections:
            reflections.append(
                MetaReflection(
                    ...
                    generated_at=timestamp,
                    source_insights=[
                        r.reflection_id for r in lower_image.meta_reflections
                    ],
                )
            )
        ...
        if lower_image.self_awareness_indicators:
            avg_indicator = sum(lower_image.self_awareness_indicators.values()) / len(
                lower_image.self_awareness_indicators
            )
            reflections.append(
                MetaReflection(
                    reflection_id=self._generate_reflection_id(level, timestamp),
    ...
    def _compute_self_awareness_indicators(
        self, lower_image: SelfImage, level: int
    ) -> Dict[str, float]:
