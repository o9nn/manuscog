...
class SimpleCognitiveAgent:
    ...
    async def cognitive_pre_think(self) -> str:
        ...
        if focus:
            focus_names = [
                getattr(a, "name", "Unknown") for a in focus if hasattr(a, "name")
            ]
            if focus_names:
            ...
            try:
                ...
                if result.insights:
                    ...
                    context_parts.append(
                        f"**Self-Reflection:** {'; '.join(insight_texts)}"
                    )
            except Exception:
        ...
        for msg in recent:
            ...
            for word in words:
                if len(word) > 5 and word.isalpha():
                    existing = self.kernel.atomspace._name_index.get_by_name(
                        word.lower()
                    )
                    if not existing:
...
async def demo_cognitive_agent():
    ...
    try:
        ...
        agent.add_to_memory(
            "user", "I need to analyze some data using Python and machine learning"
        )
        DemoOutput.info(
            "User request: 'I need to analyze some data using Python and machine learning'"
        )
        ...
        if context:
            ...
            DemoOutput.info(
                "Minimal cognitive context (expected with limited knowledge)"
            )
            ...
            try:
                ...
                if result.optimizations:
                    ...
                    for opt in result.optimizations[:2]:
                        DemoOutput.thinking(
                            f"{opt.title} (improvement: {opt.expected_improvement:.0%})"
                        )
            except Exception as e:
        ...
        if status.get("pln_stats"):
            DemoOutput.data(
                "Total PLN inferences", status["pln_stats"].get("total_inferences", 0)
            )
            DemoOutput.data(
                "New atoms created", status["pln_stats"].get("new_atoms_created", 0)
            )
