        ...
        for group_indices in groups.values():
            ...
            if self.config.mode == "mean_norm":
                ...
                std_return = math.sqrt(
                    sum((r - mean_return) ** 2 for r in group_returns) / len(group_returns)
                )
                for idx in group_indices:
                    advantages[idx] = (
                        (episode_returns[idx] - mean_return) / 
        ...
        for start, end in traj_boundaries:
            ...
            for i, step in enumerate(traj_steps):
                ...
                if step.num_inferences > 0:
                    ...
                    bonus += self.config.reasoning_bonus_weight * min(1.0, step.num_inferences / 5.0)
                
                # Attention bonus
                if step.attention_sti > 0:
                    bonus += self.config.attention_bonus_weight * min(1.0, step.attention_sti)
