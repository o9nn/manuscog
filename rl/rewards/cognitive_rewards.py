...
class CognitiveRewardManager:
    ...
    def calculate_step_reward(
        ...
        breakdown.self_improvement = self._self_improvement_reward(prev_metrics, curr_metrics)
        ...
        # Clip
        breakdown.total = max(-self.config.reward_clip, 
                             min(self.config.reward_clip, breakdown.total))
    ...
    def _reasoning_reward(
        ...
        # New inferences
        inference_delta = (
            curr.get("total_inferences", 0) - prev.get("total_inferences", 0)
        )
    ...
    def _normalize_reward(self, reward: float) -> float:
        ...
        if n > 1:
            self._running_stats["var"] = (
                self._running_stats["var"] * (n - 2) / (n - 1) +
    ...
    def get_episode_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "component_means": {
                "task": sum(b.task for b in self._episode_history) / len(self._episode_history),
                "reasoning": sum(b.reasoning for b in self._episode_history) / len(self._episode_history),
                "attention": sum(b.attention for b in self._episode_history) / len(self._episode_history),
                "growth": sum(b.growth for b in self._episode_history) / len(self._episode_history),
                "coherence": sum(b.coherence for b in self._episode_history) / len(self._episode_history),
                "self_improvement": sum(b.self_improvement for b in self._episode_history) / len(self._episode_history),
            }
        }
