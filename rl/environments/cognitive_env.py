...
class ActionSpec:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> 'ActionSpec':
        return cls(
            action=CognitiveAction(d["action"]),
            params=d.get("params", {})
        )
    ...
    def from_text(cls, text: str) -> 'ActionSpec':
        ...
        params = {}
        if action in (CognitiveAction.ADD_CONCEPT, CognitiveAction.FOCUS_ATTENTION,
            ...
            params["concept"] = param_str
        elif action in (CognitiveAction.REASON_DEDUCTION, CognitiveAction.REASON_INDUCTION,
...
class CognitiveObservation:
    ...
    def to_text(self) -> str:
        ...
        if self.optimization_opportunities > 0:
            lines.append(f"Optimization opportunities: {self.optimization_opportunities}")
        return "\n".join(lines)
...
class CognitiveEnvironment:
    ...
    def reset(self, goals: Optional[List[Dict[str, Any]]] = None) -> CognitiveObservation:
        """
    ...
    def step(self, action: ActionSpec) -> Tuple[CognitiveObservation, float, bool, Dict[str, Any]]:
        """
        ...
        # Calculate reward
        reward = self._calculate_reward(self._prev_metrics, new_metrics, action, action_result)
    ...
    def _execute_action(self, action: ActionSpec) -> Dict[str, Any]:
        ...
        try:
            if action.action == CognitiveAction.ADD_CONCEPT:
                ...
                if child and parent:
                    child_node = self.kernel.atomspace.add_node(AtomType.CONCEPT_NODE, child)
                    parent_node = self.kernel.atomspace.add_node(AtomType.CONCEPT_NODE, parent)
                    self.kernel.atomspace.add_link(
                        AtomType.INHERITANCE_LINK, [child_node, parent_node],
                        tv=TruthValue(
                ...
                concept = action.params.get("concept", "")
                if concept:
                    node = self.kernel.atomspace.get_node(AtomType.CONCEPT_NODE, concept)
                    if node:
                        sti = action.params.get("sti", 0.8)
                        self.kernel.atomspace.set_attention_value(node.handle, AttentionValue(sti=sti, lti=0.5))
                        result["success"] = True
    ...
    def _get_metrics(self) -> Dict[str, Any]:
        ...
        pln_stats = self.kernel.pln.stats if self.kernel.pln and hasattr(self.kernel.pln, 'stats') else {}
        ecan_stats = self.kernel.ecan.get_stats() if self.kernel.ecan and hasattr(self.kernel.ecan, 'get_stats') else {}
    ...
    def _calculate_reward(
        ...
        # 2. Reasoning reward
        inference_delta = new_metrics["total_inferences"] - prev_metrics["total_inferences"]
    ...
    def get_episode_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_reward": sum(self._episode_rewards) / max(1, len(self._episode_rewards)),
...
class VectorizedCognitiveEnv:
    ...
    def reset(self, goals: Optional[List[List[Dict[str, Any]]]] = None) -> List[CognitiveObservation]:
        """Reset all environments."""
        if goals is None:
            goals = [None] * self.num_envs
        return [env.reset(g) for env, g in zip(self.envs, goals)]
    
    def step(self, actions: List[ActionSpec]) -> Tuple[
