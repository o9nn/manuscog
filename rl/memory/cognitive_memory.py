...
class CognitiveMemory:
        ...
        for env_idx in range(self.batch_size):
            record = MemoryRecord(
                ...
                metadata={
                    k: records[k][env_idx]
                    for k in records
                    if k not in ("action", "text_obs", "reward")
                },
            )
            record.importance = self._calculate_importance(record)
        ...
        for env_records in data:
            ...
            if strategy == "recent":
                ...
                sorted_records = sorted(
                    env_records, key=lambda r: r.importance, reverse=True
                )
                ...
                important = sorted(
                    env_records[:-half] if len(env_records) > half else [],
    ...
    def fetch_procedures(
        ...
        sorted_procs = sorted(
            self._procedures, key=lambda p: p.get("avg_reward", 0), reverse=True
        )
