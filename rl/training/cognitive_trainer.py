...
class CognitiveRLTrainer:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "best_reward": max(h["max_reward"] for h in self.training_history)
            if self.training_history
            else 0,
            "final_success_rate": self.training_history[-1].get("success_rate", 0)
            if self.training_history
            else 0,
    ...
    def _train_epoch(self, goals: List[List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        for i in range(self.config.episodes_per_epoch):
            ...
            if self.policy and hasattr(self.policy, "update"):
                self._update_policy_from_trajectory(self.rollout.trajectories[-1])
    ...
    def _update_policy_from_trajectory(self, trajectory: List[Any]):
