        ...
        try:
            ...
            if "name" in atom_data:
                ...
                self.atomspace.add_node(
                    atom_type,
                    atom_data["name"],
                    tv=TruthValue(**atom_data.get("truth_value", {}))
                    if "truth_value" in atom_data
                    else None,
                )
            elif "outgoing" in atom_data:
                # Create link
                outgoing = [AtomHandle(uuid=h) for h in atom_data["outgoing"]]
            ...
            if "type" in query:
                ...
                if "min_strength" in query:
                    atoms = [
                        a
                        for a in atoms
                        if a.truth_value.strength >= query["min_strength"]
                    ]
                if "min_confidence" in query:
                    atoms = [
                        a
                        for a in atoms
                        if a.truth_value.confidence >= query["min_confidence"]
                    ]
...
class StatsFile(CognitiveFile):
    ...
    def __init__(
        self, path: str, atomspace: AtomSpace, stats_fn: Callable[[], Dict[str, Any]]
    ):
        super().__init__(path, atomspace)
...
class CognitiveFileSystem:
    ...
    def _setup_default_mounts(self):
        ...
        self._mounts["/cog/stats/atomspace"] = StatsFile(
            "/cog/stats/atomspace", self.atomspace, lambda: self.atomspace.get_stats()
        )

        # Query interface
        self._mounts["/cog/inference/query"] = QueryFile(
            "/cog/inference/query", self.atomspace
        )

        # Type-based atom sets
        for atom_type in AtomType:
            path = f"/cog/types/{atom_type.name.lower()}"
            self._mounts[path] = AtomSetFile(path, self.atomspace, atom_type=atom_type)
        ...
        with self._lock:
            ...
            if path in self._mounts:
                ...
                self._fd_table[fd] = FileDescriptor(
                    fd=fd, path=path, mode=mode, file_type=FileType.ATOM_SET
                )
                return fd
...
class StyxServer:
    ...
    def handle_request(self, request: StyxRequest) -> StyxResponse:
        ...
        return StyxResponse(
            msg_type=StyxMessage.RERROR, tag=request.tag, error="Unknown message type"
        )
    ...
    def _handle_walk(self, req: StyxRequest) -> StyxResponse:
        ...
        with self._lock:
            if req.fid not in self._fids:
                return StyxResponse(
                    msg_type=StyxMessage.RERROR, tag=req.tag, error="Invalid fid"
                )
        ...
        return StyxResponse(
            msg_type=StyxMessage.RWALK, tag=req.tag, data={"qids": qids}
        )
    ...
    def _handle_open(self, req: StyxRequest) -> StyxResponse:
        ...
        with self._lock:
            ...
            if fd < 0:
                return StyxResponse(
                    msg_type=StyxMessage.RERROR, tag=req.tag, error="Cannot open file"
                )
    ...
    def _handle_read(self, req: StyxRequest) -> StyxResponse:
        ...
        with self._lock:
            if req.fid not in self._fid_fds:
                return StyxResponse(
                    msg_type=StyxMessage.RERROR, tag=req.tag, error="File not open"
                )
        ...
        return StyxResponse(
            msg_type=StyxMessage.RREAD, tag=req.tag, data={"data": data}
        )
    ...
    def _handle_write(self, req: StyxRequest) -> StyxResponse:
        ...
        return StyxResponse(
            msg_type=StyxMessage.RWRITE, tag=req.tag, data={"count": count}
        )
    ...
    def _handle_clunk(self, req: StyxRequest) -> StyxResponse:
        ...
        return StyxResponse(msg_type=StyxMessage.RCLUNK, tag=req.tag)
    ...
    def _handle_stat(self, req: StyxRequest) -> StyxResponse:
        ...
        with self._lock:
            ...
            if not stat:
                return StyxResponse(
                    msg_type=StyxMessage.RERROR, tag=req.tag, error="Cannot stat"
                )

        return StyxResponse(
            msg_type=StyxMessage.RSTAT, tag=req.tag, data={"stat": stat}
        )
