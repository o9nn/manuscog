...
class ProcessTable:
    ...
    def __init__(self):
        ...
        self._by_state: Dict[CognitiveProcessState, Set[str]] = {
            s: set() for s in CognitiveProcessState
        }
    ...
    def get_by_type(self, proc_type: CogProcType) -> List[CognitiveProcess]:
        ...
        with self._lock:
            return [
                self._processes[pid]
                for pid in self._by_type[proc_type]
    ...
    def get_by_state(self, state: CognitiveProcessState) -> List[CognitiveProcess]:
        ...
        with self._lock:
            return [
                self._processes[pid]
                for pid in self._by_state[state]
                if pid in self._processes
            ]
...
class CognitiveScheduler:
    ...
    def __init__(
        self, atomspace: AtomSpace, max_concurrent: int = 4, cycle_budget: float = 10.0
    ):
    ...
    def start(self) -> bool:
        ...
        self._scheduler_thread = threading.Thread(
            target=self._scheduler_loop, daemon=True, name="cognitive-scheduler"
        )
...
class GoalDirectedProcessFactory:
    ...
    def create_attention_process(
        self, attention_fn: Callable[[CognitiveProcess, AtomSpace], Any]
    ) -> CognitiveProcess:
