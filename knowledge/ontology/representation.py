...
class Ontology:
    ...
    def _initialize_base_ontology(self):
        ...
        # Root class
        self.add_class(
            OntologyClass(name="Thing", description="The root class of all entities")
        )
    ...
    def create_instance(
        self, class_name: str, instance_name: str, properties: Dict[str, Any] = None
    ) -> Optional[AtomHandle]:
    ...
    def set_property_value(self, instance: AtomHandle, property_name: str, value: Any):
        """Set a property value for an instance."""
        self._properties.get(property_name)

        # Create property node if needed
        prop_handle = self.atomspace.add_node(
            AtomType.PREDICATE_NODE, property_name, tv=TruthValue(1.0, 1.0)
        )

        # Create value node
        if isinstance(value, (int, float)):
            value_handle = self.atomspace.add_node(
                AtomType.NUMBER_NODE, str(value), tv=TruthValue(1.0, 1.0)
            )
        elif isinstance(value, str):
            value_handle = self.atomspace.add_node(
                AtomType.CONCEPT_NODE, value, tv=TruthValue(1.0, 0.9)
            )
        ...
        list_handle = self.atomspace.add_link(
            AtomType.LIST_LINK, [instance, value_handle]
        )
    ...
    def get_property_value(
        self, instance: AtomHandle, property_name: str
    ) -> Optional[Any]:
...
class SemanticFrame:
    ...
    def create_instance(
        self, fillers: Dict[str, Any], atomspace: AtomSpace
    ) -> AtomHandle:
        ...
        instance_handle = atomspace.add_node(
            AtomType.CONCEPT_NODE, instance_name, tv=TruthValue(1.0, 0.9)
        )
        ...
        for slot_name, value in fillers.items():
            if slot_name in self.slots:
                ...
                slot_pred = atomspace.add_node(
                    AtomType.PREDICATE_NODE, f"{self.name}:{slot_name}"
                )
                ...
                list_link = atomspace.add_link(
                    AtomType.LIST_LINK, [instance_handle, value_handle]
                )
                atomspace.add_link(
                    AtomType.EVALUATION_LINK,
                    [slot_pred, list_link],
...
class FrameLibrary:
    ...
    def create_frame_instance(
        self, frame_name: str, fillers: Dict[str, Any]
    ) -> Optional[AtomHandle]:
...
class TemporalKnowledge:
    ...
    def create_interval(self, name: str, start: float, end: float) -> AtomHandle:
        """Create a temporal interval."""
        interval = TemporalInterval(start=start, end=end, name=name)

        # Create time node
        handle = self.atomspace.add_node(
            AtomType.TIME_NODE, name, tv=TruthValue(1.0, 1.0)
        )
    ...
    def add_temporal_relation(
        self, interval1: AtomHandle, relation: TemporalRelation, interval2: AtomHandle
    ) -> AtomHandle:
        """Add a temporal relation between intervals."""
        # Create relation predicate
        rel_pred = self.atomspace.add_node(
            AtomType.PREDICATE_NODE, f"temporal:{relation.name}"
        )

        # Create evaluation
        list_link = self.atomspace.add_link(AtomType.LIST_LINK, [interval1, interval2])

        return self.atomspace.add_link(
            AtomType.EVALUATION_LINK, [rel_pred, list_link], tv=TruthValue(1.0, 0.95)
        )

    def get_temporal_relation(
        self, interval1: TemporalInterval, interval2: TemporalInterval
    ) -> TemporalRelation:
        ...
        if interval1.end < interval2.start:
            ...
            return (
                ...
                else TemporalRelation.STARTED_BY
            )
...
class KnowledgeBase:
    ...
    def add_fact(
        self, subject: str, predicate: str, obj: str, confidence: float = 0.9
    ) -> AtomHandle:
        """Add a simple fact to the knowledge base."""
        # Create nodes
        subj_handle = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, subject, tv=TruthValue(1.0, 0.9)
        )

        pred_handle = self.atomspace.add_node(
            AtomType.PREDICATE_NODE, predicate, tv=TruthValue(1.0, 1.0)
        )

        obj_handle = self.atomspace.add_node(
            AtomType.CONCEPT_NODE, obj, tv=TruthValue(1.0, 0.9)
        )

        # Create evaluation
        list_link = self.atomspace.add_link(
            AtomType.LIST_LINK, [subj_handle, obj_handle]
        )
    ...
    def add_inheritance(
        self, child: str, parent: str, confidence: float = 0.95
    ) -> AtomHandle:
    ...
    def query_facts(
        ...
        for link in self.atomspace.get_atoms_by_type(AtomType.EVALUATION_LINK):
            ...
            if (
                list_atom.atom_type == AtomType.LIST_LINK
                and len(list_atom.outgoing) >= 2
            ):
