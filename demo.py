...
def demo_basic_knowledge():
    ...
    kernel.atomspace.add_link(
        AtomType.INHERITANCE_LINK, [cat, mammal], tv=TruthValue(1.0, 0.95)
    )
    kernel.atomspace.add_link(
        AtomType.INHERITANCE_LINK, [dog, mammal], tv=TruthValue(1.0, 0.95)
    )
    kernel.atomspace.add_link(
        AtomType.INHERITANCE_LINK, [mammal, animal], tv=TruthValue(1.0, 0.98)
    )
    kernel.atomspace.add_link(
    ...
    # Add similarity
    kernel.atomspace.add_link(
        AtomType.SIMILARITY_LINK, [cat, dog], tv=TruthValue(0.7, 0.8)
    )
...
def demo_pattern_recognition():
    ...
    for pattern in patterns[:5]:
        print(
            f"    Pattern: support={pattern.support}, confidence={pattern.confidence:.2f}"
        )
...
def demo_full_cognitive_cycle():
    ...
    cat = kernel.atomspace.add_node(
        AtomType.CONCEPT_NODE, "Cat", av=AttentionValue(sti=0.8, lti=0.5)
    )
    animal = kernel.atomspace.add_node(
        AtomType.CONCEPT_NODE, "Animal", av=AttentionValue(sti=0.6, lti=0.5)
    )
    kernel.atomspace.add_link(
