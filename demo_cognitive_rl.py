...
def demo_environment():
    ...
    actions = [
        ...
        ActionSpec(
            CognitiveAction.ADD_RELATIONSHIP,
            {"child": "Python", "parent": "Programming Language"},
        ),
        ActionSpec(
            CognitiveAction.ADD_RELATIONSHIP,
            {"child": "Machine Learning", "parent": "AI"},
        ),
    ...
    for i, action in enumerate(actions):
        ...
        total_reward += reward
        print(
            f"    Step {i+1}: {action.action.value:20s} → reward={reward:+.3f}, "
...
def demo_memory():
    ...
    records = [
        ...
        MemoryRecord(
            ...
            episode=1,
            timestamp=time.time(),
            action="reason_deduction",
            observation="Inferred 3 new facts",
            reward=0.9,
        ),
        MemoryRecord(
            step=4,
        ...
        MemoryRecord(
            ...
            timestamp=time.time(),
            action="reason_deduction",
            observation="Inferred 5 new facts",
            reward=1.5,
        ),
        MemoryRecord(
            step=7,
            ...
            timestamp=time.time(),
            action="reflect",
            observation="Self-awareness: 0.72",
            reward=0.1,
        ),
    ]
    ...
    for s in semantic[:3]:
        print(
            f"    {s['key']}: avg_reward={s.get('avg_reward', 0):.3f}, count={s.get('count', 0)}"
        )
    ...
    if procedures:
        print(
            f"    Best: {procedures[0]['actions']} → reward={procedures[0]['avg_reward']:.3f}"
        )
...
def demo_advantage():
    ...
    computer = CognitiveAdvantageComputer(
        AdvantageConfig(
    ...
    print(
        f"    State 'abc123': traj1.step1 vs traj2.step1 → different actions, different advantages"
    )
    print(
        f"    State 'def456': traj1.step2 vs traj2.step2 → reasoning vs adding, reasoning wins"
    )
    print(
        f"    State 'ghi789': traj1.step3 vs traj2.step3 → attention vs reasoning, compared"
    )
...
def demo_full_training():
    ...
    print(f"\n  Learning Curve:")
    print(
        f"  {'Epoch':>6} {'Mean Reward':>12} {'Success Rate':>13} {'Exploration':>12}"
    )
    print("  " + "-" * 50)
    for h in trainer.training_history:
        print(
            f"  {h['epoch']:>6d} {h['mean_reward']:>12.3f} {h['success_rate']:>12.1%} {h['exploration_rate']:>12.3f}"
        )
